...
if ($DATA_OBJ->data_type == 'get_cart') {
    ...
    $query = "select c.id as cartId, c.quantity, c.productId, p.id, p.name, p.categoryId, p.price, p.shopId, p.image, u.name as shop_name from cart c left join products p on c.productId = p.id left join shops s on p.shopId = s.id left join users u on s.userId = u.id where c.userId = :uid";
    $res = $DB->read($query, ['uid' => $userId]);
...
if ($DATA_OBJ->data_type == 'remove_from_cart') {
    ...
    $info->data_type = 'remove_from_cart';
    $info->ok = $ok;
    echo json_encode($info);
    die;
}
