...
if (isset($DATA_OBJ->data_type) && $DATA_OBJ->data_type == 'add_product') {
    try {
        ...
        // if shop role, try to find associated shop id (if shops.userId exists)
        if ($_SESSION['role'] == 'shop' && !$shopId) {
            $s = $DB->read("select id from shops where userId = ?", [$_SESSION['userid']]);
            if ($s)
                $shopId = $s[0]->id;
        }

        if (!$shopId) {
            $info->message = 'Shop not found for this user';
            echo json_encode($info);
            die;
        }
        ...
        if (isset($params['image'])) {
            $ok = $DB->write("insert into products (name, categoryId, price, quantity, shopId, image) values (?, ?, ?, ?, ?, ?)", 
                            [$name, $categoryId, $price, $quantity, $shopId, $imagePath]);
        } else {
            $ok = $DB->write("insert into products (name, categoryId, price, quantity, shopId) values (?, ?, ?, ?, ?)", 
                            [$name, $categoryId, $price, $quantity, $shopId]);
        }

        $info->data_type = 'add_product';
        $info->success = $ok ? true : false;
        $info->message = $ok ? 'Product added successfully' : 'Could not add product';
        echo json_encode($info);
        die;
    } catch (Exception $e) {
...
if ($result) {
    foreach ($result as &$product) {
        if (isset($product->image) && !empty($product->image)) {
            ...
            $product->image = '/goGrocery/assets/images/placeholder.svg';
        }
    }
}
